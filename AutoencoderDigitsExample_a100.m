...
%images.ReadFcn = @(loc)imresize(imread(loc),[100,100]);
...
images.ReadFcn = @(loc)imresize(imread(loc),[60,60]);
...
softnet = trainSoftmaxLayer(feat2,F,'MaxEpochs',10);
...
xTst = zeros(inputSize*3,length(valImages.Labels));
...
xTrn = zeros(inputSize*3,numel(trainImages.Labels));
for i = 1:numel(trainImages.Labels)
    xTrn(:,i) = Z{i}(:);
end

% Perform fine tuning
%deepnet = train(deepnet,xTrn,tTrain);
deepnet = train(deepnet,xTrn,F);
%%
